...
export const albumScrollPayload = ["keydown", "keydown"];
...
export function setProfileImage(value) {
  this.setState(prevState => {
    return {
      user: { ...prevState.user, profileImage: value.profile.profileImage },
...
export function deleteRequests(url, payload) {
  let data = {};
  axios.delete(`${API_URL}${url}`, payload, {}).catch(err => {
    let data = err.response.data;
    let status = err.response.status;
    errorHandler(data, status);
  });
